...
static ngx_int_t mp4_stbl_ptr_init(mp4_stbl_ptr_t *ptr, mp4_atom_xtts_t *atom, ngx_log_t *log);
...
static ngx_int_t mp4_stbl_ptr_advance_accum(mp4_stbl_ptr_t *ptr, uint32_t n, uint64_t *sample_no) {
	while (n >= ptr->samp_left) {
		n -= ptr->samp_left;
	...
	*sample_no = 0;
	while (sample_cnt > 0) {
		...
		if (sample_cnt >= 0) {
			...
			*sample_no += i;
			if (mp4_stbl_ptr_advance_entry(stts_ptr) != NGX_OK)
				return frame_no;
		} else {
			i = stts_ptr->samp_left + sample_cnt / stts_ptr->value;
			frame_no += i;
			*sample_no += i * stts_ptr->value;
			if (mp4_stbl_ptr_advance_n(stts_ptr, i) != NGX_OK)
				return frame_no;
		}
	}
	if (!trak->stss)
...
static mp4_atom_xtts_t *merge_xtts(mp4_atom_t *stbl, uint32_t atom_type,
                        mp4_atom_xtts_t *a, mp4_stbl_ptr_t *ap, mp4_atom_xtts_t *b,
                        uint32_t ts_a, uint32_t ts_b, uint32_t ts_d, ngx_pool_t *pool)
{
	...
	if (ap->entry < ap->end)
		do {
			...
			wr++;
		} while (mp4_stbl_ptr_advance_entry(ap) == NGX_OK);
                                   ...
                                   mp4_atom_stsc_t *b, uint32_t b_chunks, uint32_t b_offs, ngx_pool_t *pool)
{
	mp4_atom_stsc_t *x = mp4_create_table(stbl, ATOM('s', 't', 's', 'c'),
		sizeof(mp4_atom_stsc_t) + (be32toh(a->sample_cnt) + be32toh(b->sample_cnt) + 1) * 12,  pool);
...
static ngx_int_t merge_trak(mp4_trak_t *dest, mp4_trak_t *a, mp4_trak_t *b,
                            ngx_int_t frame_no, mp4_stbl_ptr_t *stts_ptr, ngx_pool_t *pool)
{
	uint32_t frame_count = be32toh(a->stsz->sample_cnt) + be32toh(b->stsz->sample_cnt), size, size2, b_offs = 0;
	uint32_t ts_a = be32toh(a->mdhd->timescale), ts_b = be32toh(b->mdhd->timescale);
	uint64_t ts_d = gcd(ts_a, ts_b); // calculate new timescale
	...
	if (a->stss && b->stss) {
		...
		for (size = 0; size < frame_count; size++)
			if (be32toh(a->stss->tbl[size]) > frame_no)
				break;
		ngx_memcpy(dest->stss->tbl, a->stss->tbl, size * 4);
		frame_count -= size;
		...
		for (rptr = a->stss->tbl; rptr < eptr; rptr++)
	...
	if (a->stsd->hdr.size != b->stsd->hdr.size || memcmp(a->stsd, b->stsd, be32toh(a->stsd->hdr.size))) {
	...
	if (!(dest->stts = merge_xtts(dest->stbl, ATOM('s', 't', 't', 's'), a->stts, stts_ptr, b->stts, ts_a, ts_b, (uint32_t)ts_d, pool)))
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	if (a->ctts && b->ctts) {
		if (mp4_stbl_ptr_init(&ctts_ptr, a->ctts, pool->log))
			return NGX_HTTP_NOT_FOUND;
		mp4_stbl_ptr_advance_n(&ctts_ptr, frame_no);
...
static ngx_int_t mp4merge_append_chunk(ngx_http_mp4merge_ctx_t *ctx, trak_alloc_info *ti, uint64_t *pos)
{
	uint32_t size = 0, chunk = ti->stsc.chunk_no;
	off_t fpos = ti->co;
	while (ti->frame_no < ti->frame_end && ti->stsc.chunk_no == chunk) {
		size += be32toh(ti->src->stsz->tbl[ti->frame_no++]);
		if (mp4_stsc_ptr_advance(&ti->stsc) != NGX_OK)
			ti->frame_no = UINT32_MAX;
	}
	if (ti->dst->co64)
		ti->dst->co->u.tbl64[ti->dst->co->chunk_cnt++] = *pos;
	else
		ti->dst->co->u.tbl[ti->dst->co->chunk_cnt++] = (uint32_t)*pos;
	...
	*pos += size;
	if (ctx->chain_last) {
		if (ctx->chain_last->buf->file_last == fpos && ctx->chain_last->buf->file == &ti->file->file) {
			ctx->chain_last->buf->file_last += size;
			return NGX_OK;
		}
	...
	ctx->chain_last->buf->file_pos = fpos;
	ctx->chain_last->buf->file_last = fpos + size;
	return NGX_OK;
}
...
static ngx_int_t mp4merge_co_alloc_seg(ngx_http_mp4merge_ctx_t *ctx,
                                     trak_alloc_info *a, trak_alloc_info *v, uint64_t *pos)
{
	ngx_int_t rc;
	while (a->frame_no < a->frame_end || v->frame_no < v->frame_end) {
		if (a->frame_no < a->frame_end && v->frame_no < v->frame_end) {
			if (a->co < v->co)
				rc = mp4merge_append_chunk(ctx, a, pos);
			else
				rc = mp4merge_append_chunk(ctx, v, pos);
...
static ngx_int_t mp4_do_merge(ngx_http_mp4merge_ctx_t *ctx)
{
	...
	if (mp4_stbl_ptr_init(&sp, ctx->main.vid->stts, ctx->req->connection->log))
		return NGX_HTTP_NOT_FOUND;
	vframe = ins_frameno(ctx->main.vid, &sp, ctx->inspos * be32toh(ctx->main.vid->mdhd->timescale), &sample_no);
	merge_trak(ctx->vid, ctx->main.vid, ctx->mixin.vid, vframe, &sp, ctx->req->pool);

	if (mp4_stbl_ptr_init(&sp, ctx->main.aid->stts, ctx->req->connection->log))
		return NGX_HTTP_NOT_FOUND;
	aframe = ins_frameno(ctx->main.aid, &sp, sample_no *
		be32toh(ctx->main.aid->mdhd->timescale) / be32toh(ctx->main.vid->mdhd->timescale), &sample_no);
	merge_trak(ctx->aid, ctx->main.aid, ctx->mixin.aid, aframe, &sp, ctx->req->pool);
	...
	n = be32toh(ctx->main.vid->co->chunk_cnt) + be32toh(ctx->mixin.vid->co->chunk_cnt) + 1;
	if (co64)
		ctx->vid->co = mp4_create_table(ctx->vid->stbl, ATOM('c', 'o', '6', '4'), sizeof(mp4_atom_stco_t) + n * 8, ctx->req->pool);
	else
		ctx->vid->co = mp4_create_table(ctx->vid->stbl, ATOM('s', 't', 'c', 'o'), sizeof(mp4_atom_stco_t) + n * 4, ctx->req->pool);
	if (!ctx->vid->co)
		return NGX_HTTP_INTERNAL_SERVER_ERROR;

	n = be32toh(ctx->main.aid->co->chunk_cnt) + be32toh(ctx->mixin.aid->co->chunk_cnt) + 1;
	if (co64)
		ctx->aid->co = mp4_create_table(ctx->aid->stbl, ATOM('c', 'o', '6', '4'), sizeof(mp4_atom_stco_t) + n * 8, ctx->req->pool);
	else
		ctx->aid->co = mp4_create_table(ctx->aid->stbl, ATOM('s', 't', 'c', 'o'), sizeof(mp4_atom_stco_t) + n * 4, ctx->req->pool);
	if (!ctx->aid->co)
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	...
	if (mp4merge_co_alloc_seg(ctx, &main_a, &main_v, &ctx->mdat_len))
		return NGX_ERROR;
	...
	if (mp4merge_co_trak_init(&mixin_v, ctx->mixin.vid, ctx->req->connection->log) != NGX_OK)
		return NGX_ERROR;
	if (mp4merge_co_alloc_seg(ctx, &mixin_a, &mixin_v, &ctx->mdat_len))
		return NGX_ERROR;
